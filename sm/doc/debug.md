...
The **cause is not yet established**. An earlier revision of this document claimed the cause was
that MIX-level SSI transaction blocking (PWR_MixSsiBlockingSet()/PWR_MixSsiBlockingUpdate()) is
skipped on Rev A in DEV_SM_PowerStateSet(). That claim is **wrong**, for two reasons that are
visible in the code:

- At boot stage 9 the DEV_SM_Init() loop only calls DEV_SM_PowerStateGet() and, for a domain that
  is already on, DEV_SM_PowerUpPost(). It never calls DEV_SM_PowerStateSet(). The NOC domain is
  already powered when the SM starts, so the SSI blocking code in DEV_SM_PowerStateSet() is not
  on the failing path at all.
- On i.MX95 PWR_MixSsiBlockingSet() and PWR_MixSsiBlockingUpdate() only act on
  PWR_MIX_SLICE_IDX_GPU; every other MIX, including NOC, hits the empty default case. Calling
  them for ::DEV_SM_PD_NOC is a no-op regardless.

Restoring the blocking on Rev A therefore could not have changed anything, and indeed the fault
reproduced with an identical signature. The Rev A code paths are left as upstream.

What remains on the failing path is DEV_SM_PowerUpPost(::DEV_SM_PD_NOC) ->
DEV_SM_NocConfigLoad(). Fault 66 is part of the SSI parity group (faults 64-69) that was masked
until SM-378 (commit 21ca4f7) set the FCCU "Faults Enabled" word[2] from `0x00000000` to
`0x0000003F`. The same commit also added NOCMIX NIU timeout writes to DEV_SM_NocConfigLoad(), so
the fault was either merely unmasked or is newly generated by those writes. Build with
`NOC_NIU_TIMEOUT=0` to tell the two apart: it skips the SM-378 writes while leaving fault 66
armed. If the board boots, the writes generate the fault; if it still faults with the same
signature, the enable word is the whole story. See `docs/nxp-escalation-fccu-fault-66.md`.
...
| NOC_NIU_TIMEOUT=0 | Skip the NOCMIX NIU timeout writes that SM-378 (21ca4f7) added to DEV_SM_NocConfigLoad() (fill of BLK_CTRL_NOCMIX NIU_TO_CTRL_* at offset 0xC0..0xEC, then an UPD pulse). Fault 66 stays armed, so this isolates "the writes generate the parity error" from "the fault was merely unmasked". Diagnostic bisect option; it drops a configuration NXP added deliberately, so it is not a fix. |
